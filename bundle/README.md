...
## Проверка бандла перед выпуском

`verify.sh` прогоняет бандл под настоящим systemd в виртуалке: установка,
`check`, служба, HTTPS, вход, живые данные из лаборатории, падение с
перезапуском, копия базы, обновление, откат, остановка. Как поднять
виртуалку — в шапке скрипта. Контейнер для этого не годится: там нет
systemd, а проверяется именно он.
