...
/// Горизонт инцидента: тот, на котором он открылся.
///
/// Инцидент, заведённый до того, как горизонт стали хранить, его не несёт.
/// Для метрик он тогда записан в сигнатуре, для логов считаем пятнадцать
/// минут: так эти инциденты и разбирались.
fn horizon(incident: &Incident) -> String {
    if !incident.horizon.is_empty() {
        return incident.horizon.clone();
    }
    incident
        .signature
        .as_str()
